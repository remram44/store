...
impl KeyPair {
    ...
    pub fn decrypt(&self, data: &[u8], counter: u32) -> (Vec<u8>, u32) {
        let mut result = Vec::new();
        let counter = self.decrypt_into(data, &mut result, counter);
        (result, counter)
    }
    ...
    /// value. That counter is used to prevent replay attacks; if the message
    /// countains a counter too low, it will be rejected.
    pub fn decrypt_into(&self, data: &[u8], result: &mut Vec<u8>, counter: u32) -> u32 {
        todo!()
    }
}
